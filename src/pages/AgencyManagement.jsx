...
import { Plus, Edit2, Trash2, X, ChevronLeft, ChevronRight, Search, RefreshCw, Building2 } from "lucide-react";
import { showSuccess, showError } from "../components/shared/toast";
import { fetchAgencies, createAgency, updateAgency, deleteAgency } from "../services/api/agencyApi";
...
const AgencyManagement = () => {
  ...
  const loadAgencies = async (page = 1) => {
    setLoading(true);
    try {
      const result = await fetchAgencies(page, pageSize);
      if (result.success) {
        setAgencies(result.data || []);
        setTotalElements(result.data?.length || 0);
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!formData.agencyName) newErrors.agencyName = "Vui lòng nhập tên đại lý";
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        resetForm();
        loadAgencies(currentPage);
      } else showError(result.message || "Có lỗi xảy ra!");
    } catch (error) {
      console.error(error);
    ...
    <div className="space-y-6 p-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            onClick={() => loadAgencies(currentPage)}
            disabled={loading}
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase">Địa chỉ</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase">SĐT</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredData.map((agency, idx) => (
                  <tr key={agency.agencyId} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-800">{agency.agencyName}</td>
                    <td className="px-6 py-4 text-sm text-gray-800">{agency.address}</td>
                    <td className="px-6 py-4 text-sm text-gray-800">{agency.phoneNumber}</td>
                    <td className="px-6 py-4 text-sm text-gray-800">{agency.email}</td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center gap-1">
                        <button onClick={() => openEditModal(agency)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Chỉnh sửa">
                          <Edit2 className="w-4 h-4" />
                        </button>
                        <button onClick={() => openDeleteConfirm(agency)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Xóa">
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/20 backdrop-blur-sm" onClick={() => { setShowModal(false); resetForm(); }}>
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-gray-800">{modalMode === "create" ? "Thêm đại lý mới" : "Chỉnh sửa đại lý"}</h2>
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Tên đại lý <span className="text-red-500">*</span></label>
                <input type="text" name="agencyName" value={formData.agencyName} onChange={(e) => setFormData({...formData, agencyName: e.target.value})} className={`w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.agencyName ? "border-red-500" : "border-gray-200"}`} />
              ...
              <div>
                ...
                <input type="text" name="address" value={formData.address} onChange={(e) => setFormData({...formData, address: e.target.value})} className={`w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.address ? "border-red-500" : "border-gray-200"}`} />
                {errors.address && <p className="text-sm text-red-500 mt-1">{errors.address}</p>}
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Số điện thoại <span className="text-red-500">*</span></label>
                <input type="text" name="phoneNumber" value={formData.phoneNumber} onChange={(e) => setFormData({...formData, phoneNumber: e.target.value})} className={`w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.phoneNumber ? "border-red-500" : "border-gray-200"}`} />
                {errors.phoneNumber && <p className="text-sm text-red-500 mt-1">{errors.phoneNumber}</p>}
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Email <span className="text-red-500">*</span></label>
                <input type="email" name="email" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} className={`w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.email ? "border-red-500" : "border-gray-200"}`} />
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => { setShowModal(false); resetForm(); }} className="px-4 py-2 rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors">Hủy</button>
              <button
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/20 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            ...
            <p className="text-gray-600 mb-6">Bạn có chắc muốn xóa đại lý <span className="font-semibold">{deleteTarget.agencyName}</span> không?</p>
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowDeleteConfirm(false)} className="px-4 py-2 rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors">Hủy</button>
              <button onClick={handleDelete} className="px-4 py-2 rounded-lg bg-red-500 hover:bg-red-600 text-white transition-colors">Xóa</button>
            </div>
          </div>
        </div>
      )}
